...
describe('General NPC REST tests', () => {
  ...
  it('can patch an NPC description by id', async () => {
    return createKW().then(keyword => {
      return createNPC(keyword)
        .then(npc => {
          return request(app)
        ...
        .then(res => {
          expect(res.body).toEqual({
            ...
            __v: 0,
            name: expect.any(String),
            description: [{ _id: expect.any(String) }]
          });
        });
    });
  });
  ...
  // it('can patch an NPC description by id', async () => {
  //   return createKW().then(keyword => {
  //     return createNPC(keyword).then(npc => {
  //       // console.log(npc.description[0]._id);
  //       return request(app)
  //         .post('api/v1/npcs/')
  //         .send({
  //           // name: npc.name,
  //           description: [{ _id: npc.description[0]._id }]
  //         })
  //         .then(res => {
  ...
  //     });
  //     // .patch(`api/v1/npcs/${npc._id}`)
  //     // .send({ description: [{ keyword: keyword._id }] });
  //   });
  //   // .then(res => {
  //   //   expect(res).toEqual({
  //   //     __v: 0
